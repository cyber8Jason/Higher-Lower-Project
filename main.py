...
Dieses Modul enthält Funktionen zur Durchführung des Wikipedia Spiels: Higher Lower
Bei diesem Spiel muss der Benutzer erraten welcher Wikipedia einen höheren Wert besitzt.
...
# Liste mit Tupeln (Land, Einwohnerzahl)
...
def main_menu():
    ...
    match user_choice1:
        ...
        case "3":
            # Zeigt Hilfe an
            print(
                "\n\033[1;34mSpielmodi:\033[1;33m\n1. \033[1;34mStandard-Modus:\033[0m\nDer klassische Spielmodus, bei dem du in jeder Runde "
                "entscheiden musst,welcher von zwei Artikeln häufiger aufgerufen wurde wobei der letzte immer "
                "übernommen wird.\n--> Drücke 1 oder 2 für den Artikel mit den HÖHEREN WERTEN."
                "\n\033[1;33m2. \033[1;34mRandom-Modus:\033[0m \nEin Modus, bei dem die Artikel zufällig ausgewählt werden, was das Spiel noch "
                "unvorhersehbarer macht.\n--> Drücke 1 oder 2 für den Artikel mit den HÖHEREN WERTEN."
                "\n\033[1;33m3. \033[1;34mGeo-Modus:\033[0m\nEin Modus, der sich auf geografische Themen konzentriert."
                "\n--> Drücke 1 oder 2 für das Land mit den MEISTEN Einwohnern."
                "\n\n\033[1;34mHighscores:\033[0m\nDein Score wird am Ende des Spiels gespeichert. Die besten Scores werden in einer "
                "Highscore-Liste angezeigt, die du im Hauptmenü einsehen kannst.\nDie Highscore-Liste zeigt die "
                "Top 5 Spieler mit den höchsten Scores.\033[0m")
            print("\033[1;33mDrücke Enter, um fortzufahren...\033[0m")
            input()
...
def gamemode_menu():
    ...
    print("\033[1;33m3.\033[0m Geo-Modus")  # Option 3: Geo-Modus
    print("=" * 66)

    # Schleife,
    #  um sicherzustellen, dass der Benutzer eine gültige Wahl trifft
    while True:
        # Eingabeaufforderung für den Benutzer zur Auswahl eines Modus
        user_choice2 = input("\n\033[1;33mBitte triff eine Wahl (1-3): \033[0m")

        # Überprüfung, ob die Eingabe eine gültige Zahl (zwischen 1 und 3) ist
        if user_choice2 in ["1", "2", "3"]:
            ...
            print("\033[1;31mUngültige Auswahl! Bitte gib eine Zahl zwischen 1 und 3 ein: \033[0m")
    print("=" * 66)
...
def standard_spiel_loop():
    ...
    while True:
        ...
        wiki_name2, wiki_num2 = wikipedia_database.get_random_article(TOP_ARTICLES, excluded=[wiki_name1])

        # Zeige die beiden Artikel an, wobei der zweite Artikel noch unbekannt ist
        print("=" * 66)
        print(f"\n\033[1;35m1. {wiki_name1}: {format_number(wiki_num1)}\n\033[0m")  # Magenta für Artikel
        ...
        print(f"\n\033[1;35m1. {wiki_name1}: {format_number(wiki_num1)}\n\033[0m")
        print(f"\033[1;35m2. {wiki_name2}: {format_number(wiki_num2)}\n\033[0m")
...
def geo_spiel_loop():
    ...
    # Verwende die globale Variable für den Highscore
    global HIGHSCORE
    country1, population1 = wikipedia_database.get_random_country(COUNTRIES_WITH_POPULATION)

    while True:
        # Generiere zwei zufällige Länder und deren Einwohnerzahl
        country2, population2 = wikipedia_database.get_random_country(COUNTRIES_WITH_POPULATION, excluded=[country1])

        # Zeige die beiden Länder an, wobei das zweite Land noch unbekannt ist
        print("=" * 66)
        print(f"\n\033[1;35m1. {country1}: {format_number(population1)}\n\033[0m")  # Magenta für Land
        print(f"\033[1;35m2. {country2}: ???\n\033[0m")  # Magenta für Land
        print("=" * 66)

        # Frage den Benutzer, welches Land mehr Einwohner hat
        while True:
            user_choice = input(
                "\033[1;33mWelches Land hat mehr Einwohner?: \033[0m")  # Eingabeaufforderung in Grün
            if user_choice == "1" or user_choice == "2":
                break  # Verlasse die Schleife, wenn eine gültige Eingabe (1 oder 2) gemacht wurde
            else:
                print("\033[1;31mFalsche Eingabe! Tippe (1) oder (2): \033[0m")  # Fehlerhinweis in Rot

        # Zeige die tatsächliche Einwohneranzahl für beide Länder
        print(f"\n\033[1;35m1. {country1}: {format_number(population1)}\n\033[0m")
        print(f"\033[1;35m2. {country2}: {format_number(population2)}\n\033[0m")

        # Überprüfe die Wahl des Benutzers
        if user_choice == "1":  # Der Benutzer hat "1" gewählt (d.h. das erste Land hat mehr Einwohner)
            if population1 > population2:
                ...
                # Fehlerbehandlung bei falscher Antwort
                print("\033[1;31mFalsch! Die Einwohnerzahl ist nicht höher.\033[0m")  # Fehlerhinweis in Rot
                print(f"\033[1;31mDein Finalscore ist: {score}\033[0m")
                print(f"\033[1;31mHIGHSCORE: {HIGHSCORE}\033[0m")
                update_highscore(USER_NAME, score)
                break  # Beende das Spiel bei falscher Antwort
        elif user_choice == "2":  # Der Benutzer hat "2" gewählt (d.h. das zweite Land hat mehr Einwohner)
            if population1 < population2:
                # Erhöhe die Rundenanzahl und den Score, wenn die Wahl richtig war
                rundenanzahl += 1
                score += 1
                ...
                # Fehlerbehandlung bei falscher Antwort
                print("\033[1;31mFalsch! Die Einwohnerzahl ist nicht niedriger.\033[0m")  # Fehlerhinweis in Rot
                print(f"\033[1;31mDein Finalscore ist: {score}\033[0m")
                print(f"\033[1;31mHIGHSCORE: {HIGHSCORE}\033[0m")
                update_highscore(USER_NAME, score)
                break  # Beende das Spiel bei falscher Antwort
...
def update_highscore(USER_NAME, score):
    ...
    print("\033[1;32mHighscore aktualisiert! Dein Name und Score wurden hinzugefügt.\033[0m")
